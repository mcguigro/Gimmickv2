...
public class BossController : MonoBehaviour {
	...
	public const float STARPOWERY = 2f;
	...
	public void handleDieing() {
		if (deathTimer < 0) {
			...
			//make some explosions or something
		}
	...
	void OnCollisionEnter2D(Collision2D other) {
		if (other.transform.tag == "Star") {
			...
			myRigidBody.velocity = new Vector3 (pushVecRaw.x * STARPOWERX, pushVecRaw.y * STARPOWERY);
			setStatus (FALLING);
	...
	public Vector3 getJumpVelocity() {
		float xVel = (float)(randObj.NextDouble () * (MAXJUMPXVEL - MINJUMPXVEL) + MINJUMPXVEL);
